...
def check_answer(coursepageDTOList):
    ...
    for parentId in parentIds:
        ...
        while l < len(questionsIdList):
            ...
            if isinstance(answerJson, list) and answerJson['correctAnswerList']:
                n = 0
                while n < len(answerJson['correctAnswerList']):
                    questions['questionid'] = answerJson['correctAnswerList'][n]['questionid']
                    questions['answerList'] = answerJson['correctAnswerList'][n]['correctAnswerList']
                    questions['score'] = 100
...
def check_section(wholepageItemDTOList, userInfo, itemsInfo):
    ...
    while i < len(wholepageItemDTOList['wholepageDTOList']):
        ...
        response = requests.request(method='POST', url="https://tao.ufec.cn/getSign.php", headers=headers,
                                    data=postJsonStr)
